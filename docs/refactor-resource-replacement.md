...
首选游戏原生 submesh/部件可见性输入。若最终使用索引隐藏变体，该变体必须保留顶点、骨骼槽位、bindpose、材质槽、Renderer 和 LOD 契约，只让目标 submesh 的索引数为零。
...
| 2 | 纯按键显隐 | 待实现/验收 | 只改变可见性，资源和生命周期指针不变 |
| 3 | F10 统一资源代际 | 待实现/验收 | 已有实例一致更新，失败可回滚，旧代际可退休 |
...
| 按键 | 连续切换 | Mesh、bones、rootBone、Animator、Physics | 目标部件可见性 |
