...
### 20.44 Blender 0.26.0：单一参数入口与内嵌浮点曲线

0.25.0 虽统一了作者组与原生组的数据模型，却同时暴露五项常用参数、角度专用区、九个 Empty 自定义属性、
Graph Editor 和 249 项可编辑源字段。源组字段沿 TypeTree 遍历顺序加入，作者快照又按路径排序，造成同一参数模板
在不同对象上排列不一致；完整源字段还会让用户误以为新增链必须手工填写数百项。

0.26.0 将对象属性收敛为一个日常编辑入口。基础参数固定按物理混合权重、重力、重力衰减、动画姿态比例和
重置后稳定时间排列。九类沿链参数通过下拉框一次选择一项，当前项只显示基础值、`useCurve`、与它直接相关的约束
开关/附加值，以及一个 Blender `Float Curve`；横轴 0～1 是根到末端，纵轴是倍率。角度限制总开关和刚度只在
选择角度曲线时同区出现。旧的 Graph Editor 按钮、九个可编辑 ID 属性和专用角度参数区不再显示。

完整模板仍保留在 Empty 的 RNA 字段集合与作者 v4 `nativeParameters` 中，用于复制、粘贴和导出；折叠区改名为
“源数据检查（只读）”，按含数字索引的稳定路径排序，支持筛选和分页，但不再提供第二套修改入口。未编辑的原生
曲线继续由原字段保存原关键帧、切线、权重和 Infinity/RotationOrder。用户实际编辑内嵌 Float Curve 后，才按当前
控制点生成新的关键帧和无权重切线；允许 2～64 个控制点。作者最小组的节点半径也改用同一内嵌控件，旧 `.blend`
中的 EIEM Action 会在重载时先写回再迁移，随后删除旧 Action 和对象曲线属性。

Blender 5.0.1 回归通过插件发现/三轮注册、最小作者组保存重开、真实 Typhoea 源图和正常 package 导入。真实源图
仍得到 11 个组、27 个有形碰撞体；正常 package 仍得到 62 个 Mesh、1 个共享 Rig 和 192 个所需 Transform。原生
精确未改导出、Float Curve 改动写回、节点半径/角度预览刷新、249 项参数与碰撞集合复制到新增组均有覆盖。
解压安装包三轮注册通过。

七个运行文件已同步到 `E:\vscode\EIEM_Blender` 和 Blender 5.0 插件目录。安装包
`bin/EIEM_Blender-0.26.0-compact-physics-ui.zip` 为 105959 字节，SHA256：
`E5FBD4652B457FCC80F80AD48749FA1BD41801114D0E4FCCE848864EC3A2611E`。BlenderMCP 热重载后的活动对象仍为
`maid.002 Skirt Physics`：249 项模板、5 个碰撞体引用和 9 个私有 Float Curve 节点保留，旧 Action 与九个曲线
自定义属性均为空；文件路径和未保存状态保持不变，未自动保存。

本节没有修改 DLL、写入游戏目录、部署 DLL 或增加游戏内 Physics 结论；独立源碰撞体转换状态未改变。

### 20.45 Blender 0.26.1：轻量曲线分布与选中节点实值

0.26.0 的内嵌 Float Curve 仍让日常参数修改进入一套过重的曲线编辑工作流。对本机安装的 Wilds Chain2
编辑器复核后，采用其“组保存共享动力学、节点显示局部结果、批量生成分布”的交互思路，但不照搬 RE Engine
的数据结构：终末地 `BeyondBoneCloth` 的每类参数仍是一份组级基础值、`useCurve` 和 Unity
`AnimationCurve`，同组多根与分支共同采样它，节点值不是独立的可导出源数据。

0.26.1 的对象属性默认只显示当前参数的基础值、根端倍率、末端倍率和线性/平滑变化。大型 Blender 曲线控件
不再显示；只有需要局部变化时才展开“高级关键点”，用数值字段编辑 0～1 的链位置与倍率。根端和末端不能
删除，中间点可增删；插入点先按当前曲线求值，因此插入动作本身不改变分布。未修改的原生曲线仍逐字段保留
原关键帧、切线、权重和元数据；实际修改倍率、插值或关键点后，才按当前投影生成新的无权重切线记录。

物理侧栏现在会在当前组的 Rig 骨骼被选中时显示该节点的 FIXED/MOVE/IGNORE 角色、按累计骨段长度归一化的
链位置、当前参数倍率和 `基础值 × 倍率` 实际值，并可直接在该链位置增加关键点。原生组沿用已经由几何位置
匹配建立的 Transform→Selection 映射缓存；作者组从显式骨骼身份与父链计算。该显示不把 SelectionData 数组
序号声明为通用骨骼身份，也不把节点采样值复制成第二份可编辑物理参数。

Blender 5.0.1 验证通过插件发现与三轮注册/卸载、最小作者链保存重开、真实 Typhoea 源图往返和正常 package
导入。真实源结果仍为 11 个物理组、27 个有形碰撞体；正常 package 仍为 62 个 Mesh、1 个共享 Rig、
11 个物理组、27 个碰撞体和 192 个所需 Transform。测试另覆盖线性/平滑切换、中间点插入/删除、端点删除拒绝
以及选中骨骼的角色/链位置采样。

七个运行文件已逐一同步到 `E:\vscode\EIEM_Blender` 和 Blender 5.0 插件目录。安装包
