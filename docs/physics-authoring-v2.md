...
状态：工作区实验格式，Blender 插件源码 0.25.0。新增组使用作者 v4，原生源图仍使用独立的 v2 格式，
新增原始 Prefab 物理数据的保留和编辑；**不是已接通的游戏 Physics Mod**。
...
- 以 CAB + PathID 保留源组件和 Transform 身份；包含完整解码字段、TypeTree、引用图、原始字节及 SHA256。
  复制到作者 v4 时只携带可编辑的数值、布尔/枚举与曲线；引用、预构建字节和 64 位 PathID 继续只由原生 v2 保留。
...
- 侧栏使用两个直接按钮：**复制参数**和**粘贴参数**。原生组之间复制全部匹配的可编辑数值/曲线字段；
  ...
  作者 v4 导出把字段写入 `nativeParameters`，并在导出前用 Empty 上当前的
...
胶囊、平面 Collider 仍按各自组件显示和引用。0.24.0 已补齐作者组对这些源碰撞体的复制、加入、移除和显隐关联；
把源 Collider 转为 DLL 可实例化配置仍属于后续运行时阶段。
...
原生 v2 仍不进入 Mesh Mod；显式传入 v2 组时在改动目标目录前报错。
组合 Mod 只接受新增作者无独立碰撞体组。旧 v1 创建/运行路径已经完成 `Render.physics` 实机验证；作者 v4
在同一路径上增加完整参数模板，当前只完成离线/宿主写入回读验证。选中作者组和同 Rig Mesh 时，
导出器生成 `[Physics…]`、共享 `[Skeleton…]` 及每个同 Rig Render 的 `physics=`；未选作者组时保持 Mesh-only。
带独立碰撞体的作者组同样在写目标前拒绝，直到 DLL 完成对应转换与实机验证。
