...
状态：已实现 DLL 解析、求值、按 Mod 隔离的按键调度、内置 Mod 管理器与同一 Mesh 上的 submesh 索引更新。2026-09-17 已部署测试 DLL 与 Typhoeus 验证包，待实机验收。
...
- 目前按键支持 F1–F24、A–Z、0–9、INSERT/DELETE/HOME/END/PAGEUP/PAGEDOWN、方向键，以及
  ...
  没有 hold、延时或手柄；持久化使用 Constants 的 `persist` 声明。
...
资源引用只能指向本 mod；所有条件分支中的 partner 引用都会在加载时静态校验，
但当前求值所得的 partner 列表可以随按键状态增删。
...
4. 重应用：先在独立数据中求得新状态；发布新动作、应用已登记实例。
   ...
   v80 增加纯 partner 列表更新路径：保留仍被新状态引用的 partner，只退休移除项并创建新增项；
   不再因一个服装显隐键而销毁同 Mod 的全部 Renderer。Mesh、Skeleton、材质等装配变化仍走完整恢复。
...
- partner 在销毁前先停用 GameObject、禁用绘制并移出 LOD；纯显隐切换只销毁新状态已移除的 partner。
